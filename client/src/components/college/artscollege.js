...
const Artscollege = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get('http://localhost:50/artscollege');
  ...
  const handleItemClick = (id) => {
    ...
    navigate(`/ArtsCollegeDetails/${id}`);
  };
          ...
          {data.map((value, index) => (
            <div className="card shadow col-lg-3 p-3"  style={{ margin: "50px", paddingBottom: "20px" }} >
              ...
              <div className="card-body">
                <h5 style={{ color: "blue", padding: "10px" }}><Link to={`/ArtsCollegeDetails/${value.id}`} style={{ textDecoration: "none", color: "inherit" }} onClick={() => handleItemClick(value.id)} className="card-title">{value.clg_name}</Link></h5>
                <h5>{value.review}</h5>
              </div>
              <h5>{value.ranking}</h5>
              <p className='text-center'><Rating name="size-small" defaultValue={4} size="small" /></p>
              <Link to={`/ArtsCollegeDetails/${value.id}`} onClick={() => handleItemClick(value.id)} class="btn btn-warning ">View More...</Link>
              <br />
